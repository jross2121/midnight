...
import type { ReminderRoute } from "./reminderLogic";
...
export async function sendTestReminder(): Promise<string> {
  ...
  return Notifications.scheduleNotificationAsync({
    content: {
      title: "Midnight reminders are ready",
      body: "Tap to open today's quest board.",
      sound: true,
      data: { route: "/(tabs)" satisfies ReminderRoute },
    },
    trigger: {
