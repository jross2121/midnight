...
export function buildStreakSummary(history: DrHistoryEntry[]): StreakSummary {
  ...
  for (let i = history.length - 1; i >= 0; i -= 1) {
    if (history[i].recoveryDay) continue;
...
export function buildPlanSummary(quests: Quest[]): PlanSummary {
  ...
  if (totalCount === 0) {
    return {
      ...
      body: "Add one easy quest and one useful task so midnight has something real to judge.",
    };
  }
...
export function buildNextDayPlan(quests: Quest[], history: DrHistoryEntry[]): NextDayPlan {
  ...
  if (quests.length === 0) {
    return {
      title: "Seed tomorrow",
      body: "Add one easy quest and one useful task before the next run starts.",
      steps: ["Add one easy quest", "Add one useful task", "Keep the board small"],
    };
  }
