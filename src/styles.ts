import { StyleSheet } from "react-native";
import { createCardSurface, createTileSurface, ui, withAlpha } from "@/src/utils/designSystem";
import type { ThemeColors } from "@/src/utils/themeContext";

export const HOME_GOLD = "#F5B84B";
export const CONTRACT_GOLD = "#D6A756";
const HOME_NAVY = "#07111F";
const HOME_NAVY_SOFT = "#101D30";
const HOME_GOLD_BORDER = "#5B421B";

export const createStyles = (colors: ThemeColors) => {
  const cardSurface = createCardSurface(colors, {
    padding: ui.spacing.card,
    radius: ui.radius.card,
  });

  const secondaryCardSurface = createCardSurface(colors, {
    padding: ui.spacing.card,
    radius: ui.radius.card,
    glowOpacity: 0.04,
    borderOpacity: ui.border.opacitySecondary,
    backgroundColor: colors.surface2,
  });
  const tileSurface = createTileSurface(colors);
  const compactTileSurface = createTileSurface(colors, {
    padding: ui.spacing.xs,
    radius: ui.radius.md,
    borderOpacity: 0.2,
    backgroundOpacity: 0.2,
  });
  const homeSectionRule = withAlpha(HOME_GOLD_BORDER, 0.62);
  const homeSectionRuleSoft = withAlpha(HOME_GOLD_BORDER, 0.46);

  return StyleSheet.create({
    safe: { flex: 1, backgroundColor: colors.bg },
    screen: { flex: 1, backgroundColor: colors.bg },
    undoBanner: {
      position: "absolute",
      left: 16,
      right: 16,
      bottom: 84,
      minHeight: 64,
      flexDirection: "row",
      alignItems: "center",
      gap: 12,
      paddingVertical: 10,
      paddingLeft: 14,
      paddingRight: 10,
      borderRadius: 12,
      borderWidth: 1,
      borderColor: withAlpha(HOME_GOLD, 0.48),
      backgroundColor: colors.surface,
      shadowColor: "#000000",
      shadowOffset: { width: 0, height: 5 },
      shadowOpacity: 0.28,
      shadowRadius: 10,
      elevation: 8,
    },
    undoBannerCopy: { flex: 1, minWidth: 0 },
    undoBannerTitle: {
      color: colors.textPrimary,
      fontSize: 13,
      lineHeight: 18,
      fontWeight: "900",
    },
    undoBannerText: {
      color: colors.textSecondary,
      fontSize: 12,
      lineHeight: 17,
      fontWeight: "700",
    },
    undoBannerButton: {
      minWidth: 62,
      minHeight: 44,
      alignItems: "center",
      justifyContent: "center",
      borderRadius: 9,
      backgroundColor: HOME_GOLD,
      paddingHorizontal: 14,
    },
    undoBannerButtonText: {
      color: colors.bg,
      fontSize: 12,
      fontWeight: "900",
    },
    firstDayHint: {
      flexDirection: "row",
      alignItems: "center",
      gap: 10,
      marginTop: 12,
      padding: 11,
      borderRadius: 9,
      borderWidth: 1,
      borderColor: withAlpha(HOME_GOLD, 0.25),
      backgroundColor: withAlpha(HOME_GOLD, 0.07),
    },
    firstDayHintCopy: { flex: 1, minWidth: 0 },
    firstDayHintTitle: {
      color: colors.textPrimary,
      fontSize: 12,
      lineHeight: 16,
      fontWeight: "900",
    },
    firstDayHintText: {
      color: colors.textSecondary,
      fontSize: 11,
      lineHeight: 16,
      fontWeight: "700",
      marginTop: 2,
    },
    termHelpButton: {
      width: 44,
      height: 44,
      borderRadius: 999,
      borderWidth: 1,
      borderColor: withAlpha(HOME_GOLD, 0.38),
      backgroundColor: withAlpha(HOME_GOLD, 0.1),
      alignItems: "center",
      justifyContent: "center",
    },
    termHelpButtonText: { color: HOME_GOLD, fontSize: 16, fontWeight: "900" },
    bgArt: {
      position: "absolute",
      top: 0,
      left: 0,
      right: 0,
      bottom: 0,
    },
    sectionBand: {
      paddingHorizontal: 0,
      paddingVertical: ui.spacing.md,
    },
    sectionBandTight: {
      paddingTop: 0,
      paddingBottom: ui.spacing.xs,
      gap: 12,
    },
    container: {
      paddingHorizontal: ui.spacing.screen,
      paddingTop: ui.spacing.screen,
      paddingBottom: ui.spacing.lg,
    },

    title: {
      ...ui.typography.title,
      color: colors.textPrimary,
      marginBottom: 0,
      fontSize: 24,
      lineHeight: 28,
      fontWeight: "900",
      letterSpacing: 0,
    },
    homeSubtitle: {
      color: colors.textTertiary,
      fontSize: 12,
      fontWeight: "600",
      marginTop: 4,
      marginBottom: 0,
      lineHeight: 16,
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    homeTopBar: {
      flexDirection: "row",
      justifyContent: "space-between",
      alignItems: "center",
      marginBottom: 0,
      paddingBottom: 12,
      borderBottomWidth: 1,
      borderBottomColor: homeSectionRule,
    },
    homeTitleWrap: {
      gap: 3,
    },
    brandTitleRow: {
      flexDirection: "row",
      alignItems: "center",
      gap: 8,
    },
    homeMetaText: {
      ...ui.typography.caption,
      color: colors.textPrimary,
      fontSize: 11,
      fontWeight: "900",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    homeMetaPill: {
      borderWidth: 1,
      borderColor: withAlpha(HOME_GOLD, 0.34),
      backgroundColor: withAlpha(HOME_GOLD, 0.1),
      borderRadius: 999,
      paddingHorizontal: ui.spacing.sm,
      paddingVertical: 6,
      shadowColor: HOME_GOLD,
      shadowOffset: { width: 0, height: 0 },
      shadowOpacity: 0.08,
      shadowRadius: 8,
      elevation: 1,
    },
    dayScoreCard: {
      ...secondaryCardSurface,
      position: "relative",
      overflow: "hidden",
      alignItems: "center",
      justifyContent: "center",
      marginBottom: ui.spacing.lg + ui.spacing.xs,
      paddingVertical: ui.spacing.md + ui.spacing.xs,
      borderColor: withAlpha(colors.accentPrimary, 0.16),
      borderWidth: 0.6,
      backgroundColor: withAlpha(colors.surface2, 0.96),
      shadowColor: colors.accentPrimary,
      shadowOffset: { width: 0, height: 0 },
      shadowOpacity: 0.04,
      shadowRadius: 8,
      elevation: 1,
    },
    dayScoreWatermarkWrap: {
      position: "absolute",
      right: -46,
      top: -44,
      opacity: 0.22,
    },
    dayScoreWatermarkSoftWrap: {
      position: "absolute",
      right: -58,
      top: -56,
      opacity: 0.15,
    },
    todayHero: {
      borderWidth: 1,
      borderColor: withAlpha(HOME_GOLD, 0.28),
      borderRadius: ui.radius.card,
      backgroundColor: withAlpha(HOME_NAVY_SOFT, 0.76),
      padding: ui.spacing.md,
      gap: 14,
      overflow: "hidden",
    },
    todayDetailsToggle: {
      minHeight: 44,
      flexDirection: "row",
      alignItems: "center",
      justifyContent: "space-between",
      gap: 8,
      borderTopWidth: 1,
      borderTopColor: withAlpha(colors.border, 0.22),
      paddingTop: 10,
    },
    todayDetailsToggleText: {
      color: HOME_GOLD,
      fontSize: 11,
      lineHeight: 15,
      fontWeight: "900",
    },
    todayHeroTop: {
      flexDirection: "row",
      alignItems: "flex-start",
      gap: ui.spacing.sm,
    },
    todayHeroCopy: {
      flex: 1,
      minWidth: 0,
    },
    todayEyebrow: {
      color: HOME_GOLD,
      fontSize: 10,
      lineHeight: 15,
      fontWeight: "900",
      textTransform: "uppercase",
    },
    todayHeadline: {
      color: colors.textPrimary,
      fontSize: 24,
      lineHeight: 29,
      fontWeight: "900",
      marginTop: 3,
    },
    todaySummary: {
      color: withAlpha(colors.textSecondary, 0.9),
      fontSize: 13,
      lineHeight: 19,
      fontWeight: "600",
      marginTop: 7,
    },
    todayScorePlate: {
      width: 76,
      minHeight: 76,
      borderRadius: ui.radius.card,
      borderWidth: 1,
      borderColor: withAlpha(HOME_GOLD, 0.34),
      backgroundColor: withAlpha(HOME_GOLD, 0.1),
      alignItems: "center",
      justifyContent: "center",
      paddingHorizontal: 6,
      flexShrink: 0,
    },
    todayScoreValue: {
      color: HOME_GOLD,
      fontSize: 25,
      lineHeight: 29,
      fontWeight: "900",
    },
    todayScoreLabel: {
      color: withAlpha(colors.textSecondary, 0.86),
      fontSize: 9,
      lineHeight: 13,
      fontWeight: "800",
      textTransform: "uppercase",
      marginTop: 1,
    },
    todayProgressTrack: {
      height: 10,
      borderRadius: 5,
      backgroundColor: withAlpha(colors.bg, 0.8),
      borderWidth: 1,
      borderColor: withAlpha(HOME_GOLD, 0.2),
      overflow: "hidden",
    },
    todayProgressFill: {
      height: "100%",
      minWidth: 0,
      borderRadius: 5,
      backgroundColor: HOME_GOLD,
    },
    todayMetricRow: {
      flexDirection: "row",
      alignItems: "stretch",
    },
    todayMetric: {
      flex: 1,
      minWidth: 0,
      alignItems: "center",
      justifyContent: "center",
      paddingHorizontal: 4,
      minHeight: 46,
    },
    todayMetricDivider: {
      width: 1,
      backgroundColor: withAlpha(colors.border, 0.32),
      marginVertical: 3,
    },
    todayMetricValue: {
      color: colors.textPrimary,
      fontSize: 16,
      lineHeight: 20,
      fontWeight: "900",
      textAlign: "center",
    },
    todayDrValue: {
      color: HOME_GOLD,
    },
    todayMetricLabel: {
      color: withAlpha(colors.textSecondary, 0.76),
      fontSize: 9,
      lineHeight: 13,
      fontWeight: "800",
      textAlign: "center",
      textTransform: "uppercase",
      marginTop: 2,
    },
    todayRankStrip: {
      borderTopWidth: 1,
      borderTopColor: withAlpha(colors.border, 0.28),
      paddingTop: 11,
      gap: 8,
    },
    todayRankCopy: {
      flexDirection: "row",
      alignItems: "center",
      justifyContent: "space-between",
      gap: ui.spacing.xs,
    },
    todayRankName: {
      color: colors.textPrimary,
      fontSize: 11,
      lineHeight: 15,
      fontWeight: "900",
    },
    todayRankNext: {
      flex: 1,
      color: withAlpha(colors.textSecondary, 0.78),
      fontSize: 10,
      lineHeight: 14,
      fontWeight: "700",
      textAlign: "right",
    },
    todayRankTrack: {
      height: 4,
      borderRadius: 2,
      backgroundColor: withAlpha(colors.bg, 0.72),
      overflow: "hidden",
    },
    todayRankFill: {
      height: "100%",
      borderRadius: 2,
      backgroundColor: HOME_GOLD,
    },
    todayRuleText: {
      color: withAlpha(colors.textSecondary, 0.72),
      fontSize: 11,
      lineHeight: 16,
      fontWeight: "600",
    },
    todayRuleRow: { flexDirection: "row", alignItems: "center", gap: 10 },
    todayRuleTextFlex: { flex: 1 },
    missionHero: {
      position: "relative",
      overflow: "visible",
      borderWidth: 0,
      borderColor: "transparent",
      backgroundColor: "transparent",
      borderRadius: 0,
      padding: 0,
      gap: ui.spacing.md,
      shadowColor: HOME_GOLD,
      shadowOffset: { width: 0, height: 0 },
      shadowOpacity: 0,
      shadowRadius: 0,
      elevation: 0,
    },
    missionHeroHeader: {
      flexDirection: "row",
      alignItems: "flex-start",
      justifyContent: "space-between",
      gap: ui.spacing.sm,
    },
    missionEyebrow: {
      ...ui.typography.caption,
      color: withAlpha(colors.textSecondary, 0.82),
      fontSize: 10,
      lineHeight: 15,
      fontWeight: "800",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    missionTitle: {
      color: colors.textPrimary,
      fontSize: 27,
      lineHeight: 31,
      fontWeight: "900",
      marginTop: 2,
    },
    missionCountdownPill: {
      borderWidth: 1,
      borderColor: withAlpha(HOME_GOLD, 0.3),
      backgroundColor: withAlpha(HOME_GOLD, 0.09),
      borderRadius: 999,
      paddingHorizontal: ui.spacing.sm,
      paddingVertical: 7,
      alignItems: "flex-end",
      minWidth: 96,
    },
    missionCountdownLabel: {
      ...ui.typography.caption,
      color: withAlpha(colors.textSecondary, 0.82),
      fontSize: 9,
      lineHeight: 12,
      fontWeight: "800",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    missionCountdownValue: {
      color: colors.textPrimary,
      fontSize: 17,
      lineHeight: 21,
      fontWeight: "900",
      marginTop: 1,
    },
    missionHeroBody: {
      flexDirection: "row",
      alignItems: "center",
      gap: ui.spacing.sm,
    },
    missionSignalGrid: {
      flexDirection: "row",
      gap: ui.spacing.xs,
      marginTop: -2,
    },
    missionSignalTile: {
      ...compactTileSurface,
      flex: 1,
      minWidth: 0,
      paddingHorizontal: ui.spacing.xs,
      paddingVertical: 9,
      borderColor: withAlpha(HOME_GOLD, 0.18),
      backgroundColor: withAlpha(colors.surface2, 0.46),
    },
    missionSignalLabel: {
      color: withAlpha(colors.textSecondary, 0.76),
      fontSize: 9,
      lineHeight: 12,
      fontWeight: "900",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    missionSignalValue: {
      color: colors.textPrimary,
      fontSize: 18,
      lineHeight: 22,
      fontWeight: "900",
      marginTop: 2,
      letterSpacing: 0,
    },
    missionRingSlot: {
      flexShrink: 0,
      transform: [{ scale: 0.86 }],
      marginLeft: -13,
      marginRight: -16,
      marginVertical: -13,
    },
    missionStatsPanel: {
      ...tileSurface,
      flex: 1,
      paddingHorizontal: ui.spacing.sm,
      paddingVertical: ui.spacing.sm,
      gap: 7,
      minWidth: 0,
      borderColor: withAlpha(HOME_GOLD, 0.24),
      backgroundColor: withAlpha(HOME_NAVY_SOFT, 0.74),
    },
    missionStatRow: {
      flexDirection: "row",
      alignItems: "flex-end",
      justifyContent: "space-between",
      gap: ui.spacing.xs,
    },
    missionStatLabel: {
      ...ui.typography.caption,
      color: withAlpha(colors.textSecondary, 0.82),
      fontSize: 10,
      lineHeight: 15,
      fontWeight: "800",
      letterSpacing: 0,
      textTransform: "uppercase",
      paddingBottom: 7,
    },
    missionDrValue: {
      ...ui.typography.drHero,
      color: HOME_GOLD,
      fontSize: 48,
      lineHeight: 52,
      fontWeight: "900",
      letterSpacing: 0,
    },
    missionScorePanel: {
      ...cardSurface,
      borderColor: withAlpha(colors.border, 0.24),
      backgroundColor: withAlpha(colors.surface2, 0.72),
      borderRadius: ui.radius.card,
      paddingHorizontal: ui.spacing.md,
      paddingVertical: ui.spacing.md,
      gap: 10,
    },
    missionScoreTopRow: {
      flexDirection: "row",
      justifyContent: "space-between",
      alignItems: "flex-end",
      gap: ui.spacing.sm,
    },
    missionScoreLabel: {
      ...ui.typography.caption,
      color: withAlpha(colors.textSecondary, 0.82),
      fontSize: 10,
      lineHeight: 15,
      fontWeight: "800",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    missionScoreValue: {
      color: colors.textPrimary,
      fontSize: 48,
      lineHeight: 52,
      fontWeight: "900",
      marginTop: 2,
    },
    missionScoreMeta: {
      color: withAlpha(colors.textSecondary, 0.86),
      fontSize: 12,
      lineHeight: 16,
      fontWeight: "800",
      letterSpacing: 0,
      paddingBottom: 6,
      textTransform: "uppercase",
    },
    missionScoreTrack: {
      height: 12,
      borderRadius: 999,
      backgroundColor: withAlpha(colors.bg, 0.78),
      overflow: "hidden",
      borderWidth: 1,
      borderColor: withAlpha(colors.border, 0.36),
    },
    missionScoreFill: {
      height: "100%",
      borderRadius: 999,
    },
    missionMetricGrid: {
      flexDirection: "row",
      gap: ui.spacing.sm,
    },
    missionMetricTile: {
      ...compactTileSurface,
      flex: 1,
      paddingHorizontal: ui.spacing.sm,
      paddingVertical: ui.spacing.sm,
      gap: 7,
      minWidth: 0,
    },
    missionMetricLabel: {
      ...ui.typography.caption,
      color: withAlpha(colors.textSecondary, 0.82),
      fontSize: 10,
      lineHeight: 15,
      fontWeight: "800",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    missionMetricValueRow: {
      flexDirection: "row",
      alignItems: "center",
      gap: ui.spacing.xs,
    },
    missionMetricValue: {
      color: colors.textPrimary,
      fontSize: 34,
      lineHeight: 38,
      fontWeight: "900",
      minWidth: 32,
    },
    missionMetricValueSmall: {
      color: colors.textPrimary,
      fontSize: 24,
      lineHeight: 28,
      fontWeight: "900",
    },
    statusHeader: {
      backgroundColor: withAlpha(colors.surface2, 0.36),
      borderWidth: 0,
      borderRadius: ui.radius.xl,
      paddingHorizontal: ui.spacing.lg,
      paddingTop: ui.spacing.sm,
      paddingBottom: ui.spacing.sm + ui.spacing.xxs,
      gap: ui.spacing.xs,
    },
    statusHeaderTop: {
      flexDirection: "column",
      alignItems: "center",
      gap: ui.spacing.xs,
      paddingBottom: ui.spacing.xs,
    },
    statusHeaderMain: {
      width: "100%",
      gap: 4,
      justifyContent: "center",
      alignItems: "center",
    },
    rankBadgeRail: {
      marginBottom: ui.spacing.xs,
      alignItems: "center",
      justifyContent: "center",
    },
    statusLabel: {
      ...ui.typography.caption,
      color: "#8B98A5",
      fontSize: 12,
      letterSpacing: 0,
    },
    statusDrValue: {
      ...ui.typography.drHero,
      color: "#E6EDF3",
      fontSize: 80,
      lineHeight: 80,
      fontWeight: "900",
      marginTop: 0,
      textAlign: "center",
      letterSpacing: 0,
    },
    statusRankBadge: {
      marginTop: 3,
      borderWidth: 1,
      borderColor: withAlpha(HOME_GOLD, 0.5),
      backgroundColor: withAlpha(HOME_GOLD, 0.14),
      borderRadius: 999,
      paddingHorizontal: ui.spacing.xs,
      paddingVertical: 3,
      shadowColor: HOME_GOLD,
      shadowOffset: { width: 0, height: 0 },
      shadowOpacity: 0.12,
      shadowRadius: 8,
      elevation: 2,
      flex: 1,
      alignItems: "center",
      minWidth: 0,
    },
    statusRankBadgeText: {
      fontSize: 11,
      fontWeight: "900",
      letterSpacing: 0,
      color: HOME_GOLD,
      textTransform: "uppercase",
      textAlign: "center",
    },
    statusRankName: {
      marginTop: 3,
      fontSize: 11,
      fontWeight: "900",
      letterSpacing: 0,
      color: "#E6EDF3",
      textTransform: "uppercase",
    },
    statusRankNext: {
      marginTop: 4,
      ...ui.typography.caption,
      color: "#8B98A5",
      fontSize: 11,
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    rankProgressBlock: {
      width: "100%",
      marginTop: ui.spacing.xs,
      gap: 6,
    },
    rankProgressCurrent: {
      ...ui.typography.caption,
      color: "#E6EDF3",
      fontSize: 12,
      fontWeight: "800",
      letterSpacing: 0,
      textTransform: "uppercase",
      textAlign: "left",
    },
    rankProgressTarget: {
      ...ui.typography.caption,
      color: "#8B98A5",
      fontSize: 11,
      fontWeight: "700",
      letterSpacing: 0,
      textAlign: "left",
    },
    rankProgressTrack: {
      height: 10,
      width: "100%",
      borderRadius: 999,
      backgroundColor: withAlpha(colors.bg, 0.9),
      borderWidth: 1,
      borderColor: withAlpha(HOME_GOLD_BORDER, 0.52),
      overflow: "hidden",
    },
    rankProgressFill: {
      height: "100%",
      borderRadius: 999,
      opacity: 0.9,
    },
    rankProgressMeta: {
      ...ui.typography.caption,
      color: "#E6EDF3",
      fontSize: 11,
      fontWeight: "600",
      letterSpacing: 0,
      textAlign: "left",
    },
    statusProgressBlock: {
      paddingTop: ui.spacing.xs,
      gap: 6,
    },
    statusProgressRow: {
      flexDirection: "row",
      justifyContent: "space-between",
      alignItems: "center",
    },
    statusProgressLabel: {
      ...ui.typography.caption,
      color: "#8B98A5",
      fontSize: 10,
      fontWeight: "700",
      letterSpacing: 0,
    },
    statusProgressValue: {
      ...ui.typography.caption,
      color: "#E6EDF3",
      fontSize: 11,
      fontWeight: "700",
      letterSpacing: 0,
    },
    statusProgressTrack: {
      height: 9,
      width: "100%",
      borderRadius: 999,
      backgroundColor: withAlpha(colors.bg, 0.9),
      borderWidth: 1,
      borderColor: withAlpha(colors.cardBorder, 0.5),
      overflow: "hidden",
    },
    statusProgressFill: {
      height: "100%",
      borderRadius: 999,
      opacity: 0.86,
    },
    statusStatsGrid: {
      flexDirection: "row",
      flexWrap: "wrap",
      justifyContent: "space-between",
      rowGap: ui.spacing.xs,
      paddingTop: ui.spacing.sm,
    },
    statusStatCell: {
      width: "49%",
      borderWidth: 1,
      borderColor: withAlpha(colors.cardBorder, 0.42),
      borderRadius: ui.radius.md,
      backgroundColor: withAlpha(colors.surface, 0.16),
      paddingVertical: 5,
      paddingHorizontal: ui.spacing.xs,
      gap: 1,
      alignItems: "flex-start",
    },
    statusStatLabel: {
      ...ui.typography.caption,
      color: "#8B98A5",
      fontSize: 9,
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    statusStatValue: {
      ...ui.typography.mono,
      fontSize: 22,
      lineHeight: 26,
      fontWeight: "800",
      color: "#E6EDF3",
      letterSpacing: 0,
    },
    statusStatValueStrong: {
      fontSize: 23,
      lineHeight: 27,
      fontWeight: "900",
    },
    devActionChip: {
      marginTop: ui.spacing.xs,
      alignSelf: "flex-start",
      borderWidth: 1,
      borderColor: colors.negative,
      borderRadius: ui.radius.sm,
      paddingVertical: ui.spacing.xxs,
      paddingHorizontal: ui.spacing.sm,
      minHeight: 44,
      justifyContent: "center",
    },
    devActionRow: {
      flexDirection: "row",
      gap: ui.spacing.xs,
      marginTop: ui.spacing.xs,
    },
    devActionText: {
      ...ui.typography.caption,
      color: colors.negative,
      fontSize: 11,
      letterSpacing: 0,
    },

    topRow: { flexDirection: "row", gap: 10, marginBottom: ui.spacing.md, flexWrap: "wrap" },
    pill: {
      backgroundColor: colors.surface,
      borderRadius: ui.radius.card,
      paddingVertical: 12,
      paddingHorizontal: 14,
      borderWidth: ui.border.width,
      borderColor: withAlpha(colors.border, ui.border.opacityCard),
      flex: 1,
      minWidth: 110,
      shadowColor: colors.text,
      shadowOffset: ui.shadow.soft.shadowOffset,
      shadowOpacity: ui.shadow.soft.shadowOpacity,
      shadowRadius: ui.shadow.soft.shadowRadius,
      elevation: ui.shadow.soft.elevation,
    },
    todayPill: {
      backgroundColor: colors.bg,
      position: "relative",
      shadowColor: colors.accentPrimary,
      shadowOffset: { width: 0, height: 4 },
      shadowOpacity: 0.12,
      shadowRadius: 10,
    },
    pillLabel: { color: colors.textSecondary, fontSize: 11, fontWeight: "600", letterSpacing: 0, textTransform: "uppercase" },
    pillValue: { color: colors.accentPrimary, fontSize: 15, fontWeight: "900", marginTop: 3, letterSpacing: 0 },
    drPrimaryPill: {
      width: "100%",
      flexBasis: "100%",
      minWidth: "100%",
      paddingVertical: ui.spacing.xl,
      paddingHorizontal: ui.spacing.md,
      borderWidth: ui.border.width,
      alignItems: "center",
      shadowColor: colors.text,
      shadowOffset: { width: 0, height: 0 },
      shadowOpacity: 0.05,
      shadowRadius: 6,
      elevation: 1,
    },
    drPrimaryValue: {
      ...ui.typography.drHero,
      fontSize: 60,
      lineHeight: 64,
      marginTop: ui.spacing.sm,
      marginBottom: ui.spacing.xs,
      textAlign: "center",
      textShadowColor: withAlpha(colors.accentPrimary, 0.28),
      textShadowOffset: { width: 0, height: 0 },
      textShadowRadius: 9,
    },
    drRankText: {
      marginTop: ui.spacing.xs,
      fontSize: 14,
      fontWeight: "800",
      letterSpacing: 0,
      textAlign: "center",
    },
    drSupportText: {
      textAlign: "center",
      marginTop: ui.spacing.xs,
      fontSize: ui.typography.body.fontSize,
      lineHeight: ui.typography.body.lineHeight,
      opacity: 0.86,
    },
    drProgressBlock: {
      width: "100%",
      marginTop: 12,
      alignItems: "center",
    },
    drProgressTrack: {
      width: "100%",
      height: 8,
      borderRadius: 999,
      borderWidth: 1,
      overflow: "hidden",
    },
    drProgressFill: {
      height: "100%",
      borderRadius: 999,
    },
    drProgressText: {
      marginTop: 6,
      fontSize: 11,
      fontWeight: "700",
      letterSpacing: 0,
    },
    drLabelText: {
      fontSize: 13,
      fontWeight: "800",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    secondaryPillLabel: {
      fontSize: 9,
      opacity: 0.75,
    },
    secondaryPillValue: {
      fontSize: 13,
      marginTop: 2,
      opacity: 0.85,
    },
    todayLabelRow: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6 },
    todayAccentDot: { width: 6, height: 6, borderRadius: 999 },
    todayContent: { zIndex: 1, alignItems: "center" },
    todayRingWrap: { position: "absolute", zIndex: 0 },
    todayXPRow: {
      alignSelf: "center",
      backgroundColor: `${colors.accentPrimary}1A`,
      borderRadius: 9,
      paddingVertical: 4,
      paddingHorizontal: 8,
      marginTop: 6,
    },
    todayValue: {
      fontSize: 14,
      textShadowColor: colors.accentPrimary,
      textShadowOffset: { width: 0, height: 0 },
      textShadowRadius: 6,
    },
    todaySummaryCard: {
      marginTop: ui.spacing.md,
      borderRadius: ui.radius.card,
      borderWidth: 0,
      paddingVertical: ui.spacing.md,
      paddingHorizontal: ui.spacing.md,
      flexDirection: "row",
      alignItems: "center",
      justifyContent: "space-between",
    },
    todaySummaryLabel: {
      fontSize: 12,
      fontWeight: "700",
      textTransform: "uppercase",
      letterSpacing: 0,
    },
    todaySummaryValue: {
      fontSize: 13,
      fontWeight: "800",
    },

    row: { flexDirection: "row", gap: 10, marginBottom: ui.spacing.md, flexWrap: "wrap" },
    homeActionRow: { flexDirection: "row", gap: ui.spacing.sm, marginTop: ui.spacing.md, marginBottom: 0 },
    btn: {
      backgroundColor: colors.surface,
      paddingVertical: 11,
      paddingHorizontal: 14,
      borderRadius: ui.radius.button,
      borderWidth: 1,
      borderColor: withAlpha(colors.border, 0.3),
      shadowColor: "#000000",
      shadowOffset: { width: 0, height: 1 },
      shadowOpacity: 0.08,
      shadowRadius: 4,
    },
    btnDanger: {
      backgroundColor: colors.bg,
      paddingVertical: 11,
      paddingHorizontal: 14,
      borderRadius: ui.radius.button,
      borderWidth: 1,
      borderColor: withAlpha(colors.accentPrimary, 0.35),
      shadowColor: "#000000",
      shadowOffset: { width: 0, height: 1 },
      shadowOpacity: 0.08,
      shadowRadius: 4,
    },
    btnText: { color: colors.textPrimary, fontWeight: "800", fontSize: 13 },
    homeActionBtn: {
      flex: 1,
      backgroundColor: colors.surface,
      borderColor: withAlpha(colors.border, 0.3),
      borderWidth: 1,
      borderRadius: ui.radius.lg,
      paddingVertical: 12,
      paddingHorizontal: 14,
      alignItems: "center",
      justifyContent: "center",
      shadowColor: "#000000",
      shadowOffset: { width: 0, height: 1 },
      shadowOpacity: 0.08,
      shadowRadius: 4,
    },
    homeActionBtnDanger: {
      flex: 1,
      backgroundColor: colors.bg,
      borderColor: withAlpha(colors.accentPrimary, 0.35),
      borderWidth: 1,
      borderRadius: ui.radius.lg,
      paddingVertical: 12,
      paddingHorizontal: 14,
      alignItems: "center",
      justifyContent: "center",
      shadowColor: "#000000",
      shadowOffset: { width: 0, height: 1 },
      shadowOpacity: 0.08,
      shadowRadius: 4,
    },
    homeActionText: { color: colors.textPrimary, fontWeight: "900", fontSize: 12, letterSpacing: 0 },

    sectionRow: {
      flexDirection: "row",
      justifyContent: "space-between",
      alignItems: "center",
      marginBottom: ui.spacing.xs,
      marginTop: 0,
    },
    section: { ...ui.typography.heading, color: colors.textPrimary },
    sectionSecondary: {
      ...ui.typography.caption,
      color: withAlpha(colors.textSecondary, 0.78),
      fontSize: 12,
      lineHeight: 15,
      textTransform: "uppercase",
      letterSpacing: 0,
      marginBottom: ui.spacing.sm,
    },
    sectionSubtext: {
      color: withAlpha(colors.textSecondary, 0.8),
      fontSize: 14,
      lineHeight: 20,
      marginBottom: 0,
      marginTop: 0,
    },
    link: { color: HOME_GOLD, fontWeight: "900", fontSize: 13, letterSpacing: 0 },
    dailySection: {
      marginTop: 0,
      paddingTop: ui.spacing.md,
      borderTopWidth: 1,
      borderTopColor: homeSectionRule,
      gap: 12,
    },
    dailyHeaderCard: {
      borderWidth: 1,
      borderLeftWidth: 3,
      borderColor: withAlpha(HOME_GOLD, 0.24),
      borderLeftColor: withAlpha(HOME_GOLD, 0.72),
      backgroundColor: withAlpha(colors.surface2, 0.52),
      borderRadius: 8,
      paddingHorizontal: ui.spacing.md,
      paddingTop: ui.spacing.md,
      paddingBottom: ui.spacing.md,
      marginBottom: 0,
      gap: 10,
    },
    queueHeaderRow: {
      flexDirection: "row",
      alignItems: "center",
      justifyContent: "space-between",
      gap: ui.spacing.sm,
    },
    queueHeaderIdentity: {
      flex: 1,
      minWidth: 0,
      flexDirection: "row",
      alignItems: "center",
      gap: ui.spacing.sm,
    },
    queueHeaderIcon: {
      width: 40,
      height: 40,
      borderRadius: 8,
      borderWidth: 1,
      borderColor: withAlpha(HOME_GOLD, 0.34),
      backgroundColor: withAlpha(HOME_GOLD, 0.1),
      alignItems: "center",
      justifyContent: "center",
      flexShrink: 0,
    },
    queueHeaderCopy: {
      flex: 1,
      minWidth: 0,
    },
    queueEyebrow: {
      color: withAlpha(colors.textSecondary, 0.82),
      fontSize: 10,
      lineHeight: 14,
      fontWeight: "900",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    queueHeadline: {
      color: colors.textPrimary,
      fontSize: 20,
      lineHeight: 24,
      fontWeight: "900",
      marginTop: 1,
    },
    queueAddButton: {
      flexDirection: "row",
      alignItems: "center",
      justifyContent: "center",
      gap: 5,
      minHeight: 44,
      borderRadius: 8,
      backgroundColor: HOME_GOLD,
      paddingHorizontal: ui.spacing.sm,
      paddingVertical: 9,
      flexShrink: 0,
    },
    queueAddButtonText: {
      color: colors.bg,
      fontSize: 12,
      lineHeight: 15,
      fontWeight: "900",
      letterSpacing: 0,
    },
    queueSummaryRow: {
      flexDirection: "row",
      gap: ui.spacing.xs,
    },
    queueSummaryChip: {
      flex: 1,
      minWidth: 0,
      minHeight: 38,
      borderRadius: 8,
      borderWidth: 1,
      flexDirection: "row",
      alignItems: "center",
      gap: 5,
      paddingHorizontal: 7,
      paddingVertical: 7,
    },
    queueSummaryDot: {
      width: 6,
      height: 6,
      borderRadius: 999,
      flexShrink: 0,
    },
    queueSummaryLabel: {
      flex: 1,
      minWidth: 0,
      color: withAlpha(colors.textSecondary, 0.84),
      fontSize: 10,
      lineHeight: 13,
      fontWeight: "900",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    queueSummaryCount: {
      fontSize: 12,
      lineHeight: 15,
      fontWeight: "900",
      letterSpacing: 0,
    },
    queueHelpText: {
      color: withAlpha(colors.textSecondary, 0.86),
      fontSize: 13,
      lineHeight: 18,
      fontWeight: "700",
    },
    queueNoticeStack: {
      gap: 4,
      borderTopWidth: 1,
      borderTopColor: withAlpha(colors.border, 0.2),
      paddingTop: 9,
    },
    queueNoticeText: {
      color: withAlpha(colors.textSecondary, 0.78),
      fontSize: 12,
      lineHeight: 17,
      fontWeight: "700",
    },
    contractPanel: {
      borderWidth: 1,
      borderColor: withAlpha(CONTRACT_GOLD, 0.24),
      backgroundColor: withAlpha(CONTRACT_GOLD, 0.045),
      borderRadius: ui.radius.card,
      paddingHorizontal: ui.spacing.md,
      paddingVertical: ui.spacing.md,
      marginTop: 0,
      marginBottom: 0,
      gap: 10,
    },
    contractHeaderRow: {
      flexDirection: "row",
      justifyContent: "space-between",
      alignItems: "center",
      gap: ui.spacing.sm,
    },
    contractTitleRow: {
      flexDirection: "row",
      alignItems: "center",
      gap: ui.spacing.xs,
      flex: 1,
      minWidth: 0,
    },
    contractCopy: {
      flex: 1,
      minWidth: 0,
    },
    contractArtBadge: {
      width: 34,
      height: 34,
      borderRadius: 12,
      borderWidth: 1,
      borderColor: withAlpha(CONTRACT_GOLD, 0.34),
      backgroundColor: withAlpha(CONTRACT_GOLD, 0.1),
      alignItems: "center",
      justifyContent: "center",
      flexShrink: 0,
    },
    contractEyebrow: {
      ...ui.typography.caption,
      color: colors.textSecondary,
      fontSize: 10,
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    contractTitle: {
      color: colors.textPrimary,
      fontSize: 18,
      lineHeight: 22,
      fontWeight: "900",
      marginTop: 2,
    },
    contractCounterPill: {
      borderWidth: 1,
      borderColor: withAlpha(CONTRACT_GOLD, 0.5),
      backgroundColor: withAlpha(CONTRACT_GOLD, 0.14),
      borderRadius: 999,
      paddingHorizontal: ui.spacing.sm,
      paddingVertical: 5,
    },
    contractCounterText: {
      color: CONTRACT_GOLD,
      fontSize: 11,
      fontWeight: "900",
      letterSpacing: 0,
    },
    contractProgressTrack: {
      height: 9,
      borderRadius: 999,
      backgroundColor: withAlpha(colors.bg, 0.8),
      overflow: "hidden",
      borderWidth: 1,
      borderColor: withAlpha(CONTRACT_GOLD, 0.3),
    },
    contractProgressFill: {
      height: "100%",
      borderRadius: 999,
    },
    contractStatusText: {
      color: withAlpha(colors.textSecondary, 0.86),
      fontSize: 12,
      lineHeight: 17,
      fontWeight: "600",
    },
    morningPlanPanel: {
      borderTopWidth: 1,
      borderTopColor: homeSectionRuleSoft,
      paddingTop: ui.spacing.md,
      marginTop: ui.spacing.xs,
      gap: ui.spacing.sm,
    },
    morningPlanHeader: {
      flexDirection: "row",
      justifyContent: "space-between",
      alignItems: "flex-start",
      gap: ui.spacing.sm,
    },
    morningPlanTitleRow: {
      flex: 1,
      flexDirection: "row",
      alignItems: "center",
      gap: ui.spacing.xs,
      minWidth: 0,
    },
    morningPlanTitleText: {
      flex: 1,
      minWidth: 0,
    },
    morningPlanArtBadge: {
      width: 34,
      height: 34,
      borderRadius: 12,
      borderWidth: 1,
      borderColor: withAlpha(HOME_GOLD, 0.3),
      backgroundColor: withAlpha(HOME_GOLD, 0.1),
      alignItems: "center",
      justifyContent: "center",
      flexShrink: 0,
    },
    morningPlanTitle: {
      color: colors.textPrimary,
      fontSize: 18,
      lineHeight: 22,
      fontWeight: "900",
      marginTop: 2,
    },
    morningPlanDeltaPill: {
      borderWidth: 1,
      borderColor: withAlpha(HOME_GOLD, 0.3),
      backgroundColor: withAlpha(HOME_GOLD, 0.09),
      borderRadius: 999,
      paddingHorizontal: ui.spacing.sm,
      paddingVertical: 6,
      alignItems: "flex-end",
      minWidth: 62,
    },
    morningPlanDeltaLabel: {
      color: withAlpha(colors.textSecondary, 0.84),
      fontSize: 9,
      lineHeight: 11,
      fontWeight: "800",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    morningPlanDeltaValue: {
      color: colors.textPrimary,
      fontSize: 16,
      lineHeight: 19,
      fontWeight: "900",
    },
    morningPlanBody: {
      color: withAlpha(colors.textSecondary, 0.9),
      fontSize: 13,
      lineHeight: 18,
      fontWeight: "600",
    },
    morningPlanMetricRow: {
      flexDirection: "row",
      gap: ui.spacing.xs,
      marginTop: 2,
    },
    morningPlanMetric: {
      ...compactTileSurface,
      flex: 1,
      paddingHorizontal: ui.spacing.xs,
      paddingVertical: 7,
      minWidth: 0,
    },
    morningPlanMetricValue: {
      color: colors.textPrimary,
      fontSize: 15,
      lineHeight: 18,
      fontWeight: "900",
    },
    morningPlanMetricLabel: {
      color: withAlpha(colors.textSecondary, 0.78),
      fontSize: 9,
      lineHeight: 12,
      fontWeight: "800",
      letterSpacing: 0,
      textTransform: "uppercase",
      marginTop: 1,
    },
    nextMoveCard: {
      ...cardSurface,
      borderColor: withAlpha(HOME_GOLD, 0.34),
      backgroundColor: withAlpha(colors.surface2, 0.88),
      borderRadius: ui.radius.card,
      paddingHorizontal: ui.spacing.md,
      paddingVertical: ui.spacing.md,
      marginBottom: 0,
      gap: 10,
      shadowColor: HOME_GOLD,
      shadowOffset: { width: 0, height: 0 },
      shadowOpacity: 0,
      shadowRadius: 0,
      elevation: 0,
    },
    nextMoveTopRow: {
      flexDirection: "row",
      alignItems: "flex-start",
      justifyContent: "space-between",
      gap: ui.spacing.sm,
    },
    nextMoveIdentity: {
      flex: 1,
      flexDirection: "row",
      alignItems: "center",
      gap: ui.spacing.xs,
      minWidth: 0,
    },
    nextMoveTextWrap: {
      flex: 1,
      minWidth: 0,
    },
    nextMoveArtBadge: {
      width: 42,
      height: 42,
      borderRadius: 14,
      borderWidth: 1,
      alignItems: "center",
      justifyContent: "center",
      flexShrink: 0,
    },
    nextMoveEyebrow: {
      ...ui.typography.caption,
      color: HOME_GOLD,
      fontSize: 10,
      lineHeight: 15,
      fontWeight: "900",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    nextMoveTitle: {
      color: colors.textPrimary,
      fontSize: 21,
      lineHeight: 25,
      fontWeight: "900",
      marginTop: 2,
    },
    nextMoveButton: {
      borderRadius: ui.radius.button,
      backgroundColor: HOME_GOLD,
      borderWidth: 1,
      borderColor: withAlpha(HOME_GOLD, 0.5),
      paddingHorizontal: ui.spacing.md,
      paddingVertical: 11,
      minWidth: 96,
      minHeight: 44,
      alignItems: "center",
      justifyContent: "center",
      shadowColor: HOME_GOLD,
      shadowOffset: { width: 0, height: 0 },
      shadowOpacity: 0,
      shadowRadius: 0,
      elevation: 0,
    },
    nextMoveButtonWide: {
      minHeight: 48,
      borderRadius: ui.radius.button,
      backgroundColor: HOME_GOLD,
      flexDirection: "row",
      alignItems: "center",
      justifyContent: "center",
      gap: 8,
      paddingHorizontal: ui.spacing.md,
    },
    nextMoveButtonText: {
      color: HOME_NAVY,
      fontSize: 12,
      fontWeight: "900",
      letterSpacing: 0,
    },
    nextMoveCompletePill: {
      borderWidth: 1,
      borderColor: withAlpha(HOME_GOLD, 0.42),
      backgroundColor: withAlpha(HOME_GOLD, 0.12),
      borderRadius: 999,
      paddingHorizontal: ui.spacing.sm,
      paddingVertical: 7,
      alignItems: "center",
      justifyContent: "center",
    },
    nextMoveCompleteRow: {
      minHeight: 44,
      flexDirection: "row",
      alignItems: "center",
      gap: 8,
    },
    nextMoveCompleteText: {
      color: HOME_GOLD,
      fontSize: 11,
      fontWeight: "900",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    nextMoveBadge: {
      alignSelf: "flex-start",
      flexDirection: "row",
      alignItems: "center",
      gap: 5,
      borderWidth: 1,
      borderColor: withAlpha(CONTRACT_GOLD, 0.36),
      backgroundColor: withAlpha(CONTRACT_GOLD, 0.1),
      borderRadius: 999,
      paddingHorizontal: 9,
      paddingVertical: 4,
    },
    nextMoveBadgeText: {
      color: CONTRACT_GOLD,
      fontSize: 10,
      lineHeight: 15,
      fontWeight: "900",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    nextMoveMeta: {
      color: withAlpha(colors.textSecondary, 0.84),
      fontSize: 11,
      lineHeight: 15,
      fontWeight: "800",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    nextMoveReason: {
      color: withAlpha(colors.textSecondary, 0.86),
      fontSize: 13,
      lineHeight: 18,
      fontWeight: "600",
    },
    focusSprintCard: {
      position: "relative",
      overflow: "hidden",
      borderWidth: 1,
      borderColor: withAlpha(HOME_GOLD, 0.24),
      backgroundColor: withAlpha(HOME_NAVY, 0.34),
      borderRadius: ui.radius.card,
      paddingHorizontal: ui.spacing.md,
      paddingVertical: ui.spacing.md,
      marginBottom: ui.spacing.lg,
      gap: ui.spacing.sm,
    },
    focusSprintAccentRail: {
      position: "absolute",
      left: 0,
      top: 0,
      bottom: 0,
      width: 4,
      backgroundColor: HOME_GOLD,
      opacity: 0.86,
    },
    focusSprintTopRow: {
      flexDirection: "row",
      alignItems: "center",
      justifyContent: "space-between",
      gap: ui.spacing.sm,
    },
    focusSprintTitleWrap: {
      flex: 1,
      minWidth: 0,
    },
    focusSprintEyebrow: {
      ...ui.typography.caption,
      color: HOME_GOLD,
      fontSize: 10,
      lineHeight: 15,
      fontWeight: "900",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    focusSprintSubtitle: {
      color: withAlpha(colors.textSecondary, 0.78),
      fontSize: 12,
      lineHeight: 16,
      fontWeight: "700",
      marginTop: 2,
    },
    focusStatusPill: {
      borderWidth: 1,
      borderColor: withAlpha(colors.border, 0.28),
      backgroundColor: withAlpha(colors.surface2, 0.48),
      borderRadius: 999,
      paddingHorizontal: 10,
      paddingVertical: 6,
      alignItems: "center",
      justifyContent: "center",
      maxWidth: 132,
    },
    focusStatusPillRunning: {
      borderColor: withAlpha(HOME_GOLD, 0.5),
      backgroundColor: withAlpha(HOME_GOLD, 0.14),
    },
    focusStatusPillComplete: {
      borderColor: withAlpha(colors.positive, 0.46),
      backgroundColor: withAlpha(colors.positive, 0.12),
    },
    focusStatusText: {
      color: withAlpha(colors.textSecondary, 0.86),
      fontSize: 9,
      lineHeight: 11,
      fontWeight: "900",
      letterSpacing: 0,
      textTransform: "uppercase",
      textAlign: "center",
    },
    focusStatusTextRunning: {
      color: HOME_GOLD,
    },
    focusStatusTextComplete: {
      color: colors.positive,
    },
    focusTimerConsole: {
      minHeight: 98,
      borderRadius: ui.radius.md,
      borderWidth: 1,
      borderColor: withAlpha(HOME_GOLD, 0.28),
      backgroundColor: withAlpha(HOME_NAVY_SOFT, 0.72),
      alignItems: "center",
      justifyContent: "center",
      paddingHorizontal: ui.spacing.md,
      paddingVertical: ui.spacing.sm,
    },
    focusSprintTitle: {
      color: colors.textPrimary,
      fontSize: 17,
      lineHeight: 21,
      fontWeight: "900",
      marginTop: 2,
    },
    focusSprintTimer: {
      color: HOME_GOLD,
      fontSize: 44,
      lineHeight: 50,
      fontWeight: "900",
      letterSpacing: 0,
      fontFamily: "monospace",
    },
    focusSprintTimerLabel: {
      color: withAlpha(colors.textSecondary, 0.76),
      fontSize: 10,
      lineHeight: 15,
      fontWeight: "900",
      letterSpacing: 0,
      textTransform: "uppercase",
      marginTop: 2,
    },
    focusProgressTrack: {
      height: 6,
      borderRadius: 999,
      backgroundColor: withAlpha(colors.surface, 0.72),
      overflow: "hidden",
    },
    focusProgressFill: {
      height: "100%",
      borderRadius: 999,
    },
    focusTargetStrip: {
      flexDirection: "row",
      alignItems: "flex-start",
      gap: ui.spacing.xs,
      borderWidth: 1,
      borderColor: withAlpha(colors.border, 0.22),
      backgroundColor: withAlpha(colors.surface2, 0.42),
      borderRadius: ui.radius.md,
      paddingHorizontal: ui.spacing.sm,
      paddingVertical: ui.spacing.xs,
    },
    focusTargetDot: {
      width: 9,
      height: 9,
      borderRadius: 999,
      backgroundColor: HOME_GOLD,
      marginTop: 5,
      shadowColor: HOME_GOLD,
      shadowOffset: { width: 0, height: 0 },
      shadowOpacity: 0.36,
      shadowRadius: 6,
      elevation: 1,
    },
    focusTargetTextWrap: {
      flex: 1,
      minWidth: 0,
    },
    focusTargetLabel: {
      color: withAlpha(colors.textSecondary, 0.72),
      fontSize: 9,
      lineHeight: 11,
      fontWeight: "900",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    focusSprintMeta: {
      color: withAlpha(colors.textSecondary, 0.82),
      fontSize: 10,
      lineHeight: 15,
      fontWeight: "900",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    focusDurationRow: {
      flexDirection: "row",
      gap: ui.spacing.xs,
    },
    focusDurationChip: {
      flex: 1,
      minHeight: 34,
      borderRadius: ui.radius.button,
      borderWidth: 1,
      borderColor: withAlpha(colors.border, 0.24),
      backgroundColor: withAlpha(colors.bg, 0.3),
      alignItems: "center",
      justifyContent: "center",
      paddingHorizontal: ui.spacing.xs,
      paddingVertical: 7,
    },
    focusDurationChipActive: {
      borderColor: withAlpha(HOME_GOLD, 0.44),
      backgroundColor: withAlpha(HOME_GOLD, 0.12),
    },
    focusDurationChipDisabled: {
      opacity: 0.58,
    },
    focusDurationText: {
      color: withAlpha(colors.textSecondary, 0.86),
      fontSize: 11,
      lineHeight: 14,
      fontWeight: "900",
      letterSpacing: 0,
    },
    focusDurationTextActive: {
      color: colors.textPrimary,
    },
    focusActionRow: {
      flexDirection: "row",
      gap: ui.spacing.xs,
    },
    focusPrimaryButton: {
      flex: 1.1,
      minHeight: 40,
      borderRadius: ui.radius.button,
      backgroundColor: HOME_GOLD,
      alignItems: "center",
      justifyContent: "center",
      paddingHorizontal: ui.spacing.sm,
      paddingVertical: 10,
    },
    focusPrimaryButtonText: {
      color: colors.bg,
      fontSize: 12,
      lineHeight: 15,
      fontWeight: "900",
      letterSpacing: 0,
    },
    focusSecondaryButton: {
      flex: 1,
      minHeight: 40,
      borderRadius: ui.radius.button,
      borderWidth: 1,
      borderColor: withAlpha(colors.border, 0.28),
      backgroundColor: withAlpha(colors.bg, 0.3),
      alignItems: "center",
      justifyContent: "center",
      paddingHorizontal: ui.spacing.xs,
      paddingVertical: 10,
    },
    focusSecondaryButtonText: {
      color: colors.textSecondary,
      fontSize: 12,
      lineHeight: 15,
      fontWeight: "900",
      letterSpacing: 0,
    },
    focusCompleteButton: {
      borderColor: withAlpha(colors.positive, 0.38),
      backgroundColor: withAlpha(colors.positive, 0.12),
    },
    focusCompleteButtonText: {
      color: colors.positive,
    },
    focusButtonDisabled: {
      opacity: 0.45,
    },
    templatePanel: {
      borderWidth: 0,
      borderTopWidth: 1,
      borderTopColor: homeSectionRuleSoft,
      paddingTop: ui.spacing.md,
      marginBottom: ui.spacing.lg,
    },
    templateHint: {
      color: withAlpha(colors.textSecondary, 0.72),
      fontSize: 11,
      fontWeight: "800",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    templateRail: {
      gap: ui.spacing.xs,
      paddingRight: ui.spacing.screen,
      paddingBottom: 2,
    },
    templateChip: {
      ...compactTileSurface,
      width: 142,
      minHeight: 102,
      paddingHorizontal: ui.spacing.sm,
      paddingVertical: ui.spacing.sm,
      justifyContent: "space-between",
    },
    templateTopRow: {
      flexDirection: "row",
      alignItems: "center",
      justifyContent: "space-between",
      marginBottom: ui.spacing.xs,
    },
    templateArtBadge: {
      width: 30,
      height: 30,
      borderRadius: 10,
      borderWidth: 1,
      alignItems: "center",
      justifyContent: "center",
    },
    templateArtGlyph: {
      fontSize: 16,
      lineHeight: 20,
    },
    templateContractMark: {
      width: 24,
      height: 24,
      borderRadius: 12,
      alignItems: "center",
      justifyContent: "center",
      backgroundColor: withAlpha(CONTRACT_GOLD, 0.1),
      borderWidth: 1,
      borderColor: withAlpha(CONTRACT_GOLD, 0.26),
    },
    templateTitle: {
      color: colors.textPrimary,
      fontSize: 13,
      lineHeight: 17,
      fontWeight: "800",
    },
    templateMeta: {
      color: withAlpha(colors.textSecondary, 0.78),
      fontSize: 10,
      lineHeight: 15,
      fontWeight: "800",
      letterSpacing: 0,
      textTransform: "uppercase",
      marginTop: ui.spacing.xs,
    },

    addBox: {
      ...cardSurface,
      marginBottom: ui.spacing.md,
      borderRadius: ui.radius.card,
      padding: ui.spacing.md,
      gap: 12,
    },
    addFormHeader: {
      flexDirection: "row",
      alignItems: "flex-start",
      justifyContent: "space-between",
      gap: ui.spacing.sm,
      paddingBottom: 2,
    },
    editorTitleRow: {
      flex: 1,
      minWidth: 0,
      flexDirection: "row",
      alignItems: "center",
      gap: 10,
    },
    editorHeroMark: {
      width: 38,
      height: 38,
      borderRadius: ui.radius.sm,
      borderWidth: 1,
      alignItems: "center",
      justifyContent: "center",
      flexShrink: 0,
    },
    editorHeroEmoji: {
      fontSize: 20,
      lineHeight: 24,
    },
    addFormTitleWrap: {
      flex: 1,
      minWidth: 0,
      gap: 3,
    },
    addFormKicker: {
      fontSize: 10,
      lineHeight: 15,
      fontWeight: "900",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    addFormTitle: {
      fontSize: 18,
      lineHeight: 22,
      fontWeight: "900",
      letterSpacing: 0,
    },
    addFormBadge: {
      borderWidth: 1,
      borderRadius: 999,
      paddingHorizontal: 10,
      paddingVertical: 6,
      minWidth: 58,
      alignItems: "center",
    },
    addFormBadgeText: {
      fontSize: 11,
      lineHeight: 14,
      fontWeight: "900",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    addFormHeaderActions: {
      flexDirection: "row",
      alignItems: "center",
      gap: 8,
      flexShrink: 0,
    },
    addFormCloseButton: {
      width: 34,
      height: 34,
      borderRadius: 999,
      alignItems: "center",
      justifyContent: "center",
      borderWidth: 1,
    },
    addFormField: {
      gap: 7,
    },
    addFormSectionHeader: {
      flexDirection: "row",
      alignItems: "center",
      justifyContent: "space-between",
      gap: ui.spacing.sm,
      marginTop: 2,
    },
    addFormHint: {
      fontSize: 11,
      lineHeight: 14,
      fontWeight: "700",
    },
    input: {
      backgroundColor: colors.bg,
      color: colors.textPrimary,
      minHeight: 48,
      paddingHorizontal: 12,
      paddingVertical: 11,
      borderRadius: ui.radius.button,
      borderWidth: 1,
      borderColor: withAlpha(colors.border, 0.32),
      fontSize: 14,
      lineHeight: 18,
      fontWeight: "800",
    },
    inputFocused: {
      borderColor: withAlpha(colors.accentPrimary, 0.55),
      backgroundColor: withAlpha(colors.surface2, 0.82),
      shadowColor: colors.accentPrimary,
      shadowOffset: { width: 0, height: 0 },
      shadowOpacity: 0.12,
      shadowRadius: 4,
    },
    smallLabel: {
      color: colors.textSecondary,
      fontWeight: "900",
      fontSize: 11,
      lineHeight: 14,
      letterSpacing: 0,
      textTransform: "uppercase",
    },

    pickerRow: { flexDirection: "row", flexWrap: "wrap", gap: 8 },
    pillPick: {
      minHeight: 44,
      paddingVertical: 9,
      paddingHorizontal: 12,
      borderRadius: ui.radius.button,
      borderWidth: 1,
      borderColor: withAlpha(colors.border, 0.32),
      backgroundColor: colors.bg,
      alignItems: "center",
      justifyContent: "center",
    },
    editorChipEmoji: {
      fontSize: 16,
      lineHeight: 19,
      marginBottom: 2,
    },
    editorChipSelected: {
      borderWidth: 1.2,
    },
    pillPickActive: { backgroundColor: withAlpha(colors.accentPrimary, 0.12), borderColor: withAlpha(colors.accentPrimary, 0.38) },
    pillPickText: {
      color: colors.textSecondary,
      fontWeight: "900",
      fontSize: 12,
      lineHeight: 15,
    },
    pillPickTextActive: { color: colors.textPrimary, fontWeight: "900" },
    weekdayPick: {
      flexGrow: 1,
      flexBasis: "20%",
      minWidth: 44,
      paddingHorizontal: 6,
    },

    addBtn: {
      backgroundColor: colors.accentPrimary,
      minHeight: 52,
      paddingVertical: 14,
      paddingHorizontal: 16,
      borderRadius: ui.radius.button,
      alignItems: "center",
      justifyContent: "center",
      shadowColor: colors.accentPrimary,
      shadowOffset: { width: 0, height: 4 },
      shadowOpacity: 0.12,
      shadowRadius: 8,
      marginTop: 2,
    },
    addBtnText: {
      color: colors.textPrimary,
      fontWeight: "900",
      fontSize: 15,
      lineHeight: 19,
      letterSpacing: 0,
    },

    editSheetModalRoot: {
      flex: 1,
      justifyContent: "flex-end",
    },
    editSheetBackdrop: {
      ...StyleSheet.absoluteFillObject,
    },
    editSheetKeyboardWrap: {
      flex: 1,
      justifyContent: "flex-end",
    },
    editSheetContainer: {
      width: "100%",
      maxHeight: "88%",
      borderTopLeftRadius: ui.radius.xl,
      borderTopRightRadius: ui.radius.xl,
      borderBottomLeftRadius: 0,
      borderBottomRightRadius: 0,
      borderWidth: 1,
      borderBottomWidth: 0,
      overflow: "hidden",
    },
    editSheetGrabber: {
      alignSelf: "center",
      width: 42,
      height: 4,
      borderRadius: 999,
      marginTop: 9,
      marginBottom: 4,
    },
    editSheetHeader: {
      flexDirection: "row",
      alignItems: "center",
      justifyContent: "space-between",
      paddingHorizontal: 14,
      paddingTop: 8,
      paddingBottom: 11,
      borderBottomWidth: 1,
      gap: 10,
    },
    editSheetTitleGroup: {
      flex: 1,
      minWidth: 0,
      gap: 3,
    },
    editSheetKicker: {
      fontSize: 10,
      lineHeight: 15,
      fontWeight: "900",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    editSheetTitle: {
      fontSize: 21,
      lineHeight: 25,
      fontWeight: "900",
      letterSpacing: 0,
    },
    editSheetCloseButton: {
      width: 36,
      height: 36,
      borderRadius: 999,
      alignItems: "center",
      justifyContent: "center",
      borderWidth: 1,
    },
    editSheetCloseText: {
      fontSize: 22,
      lineHeight: 22,
      fontWeight: "500",
      marginTop: 0,
    },
    editSheetSummaryRow: {
      flexDirection: "row",
      flexWrap: "wrap",
      gap: 7,
      paddingHorizontal: 14,
      paddingVertical: 8,
      borderBottomWidth: 1,
    },
    editSheetMetaPill: {
      minHeight: 24,
      borderRadius: 999,
      borderWidth: 1,
      paddingHorizontal: 10,
      paddingVertical: 5,
      flexDirection: "row",
      alignItems: "center",
      gap: 5,
      justifyContent: "center",
      maxWidth: "48%",
    },
    editSheetMetaEmoji: {
      fontSize: 12,
      lineHeight: 14,
      flexShrink: 0,
    },
    editSheetMetaPillText: {
      fontSize: 10,
      lineHeight: 15,
      fontWeight: "900",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    editSheetScroll: {
      flexGrow: 0,
    },
    editSheetFormContent: {
      paddingHorizontal: 12,
      paddingTop: 10,
      paddingBottom: 12,
      gap: 9,
    },
    editSheetSectionCard: {
      borderRadius: ui.radius.card,
      borderWidth: 1,
      paddingHorizontal: 12,
      paddingVertical: 12,
      gap: 12,
    },
    editSheetSectionHeaderLine: {
      flexDirection: "row",
      alignItems: "center",
      gap: 10,
      marginBottom: 2,
    },
    editSheetSectionTitle: {
      fontSize: 18,
      lineHeight: 22,
      fontWeight: "900",
      letterSpacing: 0,
    },
    editSheetSectionRule: {
      flex: 1,
      height: 1,
      borderRadius: 999,
    },
    editSheetFieldBlock: {
      gap: 7,
    },
    editSheetLabel: {
      fontSize: 11,
      lineHeight: 14,
      fontWeight: "900",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    editSheetInput: {
      minHeight: 44,
      borderRadius: ui.radius.button,
      paddingHorizontal: 12,
      paddingVertical: 11,
      borderWidth: 0,
      fontSize: 14,
      lineHeight: 18,
      fontWeight: "800",
    },
    editSheetChipRow: {
      flexDirection: "row",
      flexWrap: "wrap",
      justifyContent: "space-between",
      rowGap: 8,
    },
    editSheetDifficultyRow: {
      flexDirection: "row",
      gap: 7,
    },
    editSheetChip: {
      width: "32%",
      borderRadius: ui.radius.button,
      paddingHorizontal: 8,
      paddingVertical: 8,
      minHeight: 50,
      alignItems: "center",
      justifyContent: "center",
      borderWidth: 1,
      overflow: "hidden",
    },
    editSheetDifficultyChip: {
      borderRadius: ui.radius.button,
      paddingHorizontal: 8,
      paddingVertical: 8,
      minHeight: 50,
      flex: 1,
      alignItems: "center",
      justifyContent: "center",
      borderWidth: 1,
      overflow: "hidden",
    },
    editSheetWeekdayRow: {
      flexDirection: "row",
      flexWrap: "wrap",
      gap: 6,
      marginTop: 7,
    },
    editSheetWeekdayChip: {
      flexGrow: 1,
      flexBasis: "22%",
      minWidth: 44,
      borderRadius: ui.radius.button,
      paddingHorizontal: 4,
      paddingVertical: 8,
      minHeight: 44,
      alignItems: "center",
      justifyContent: "center",
      borderWidth: 1,
    },
    editSheetChipText: {
      fontSize: 12,
      lineHeight: 15,
      fontWeight: "900",
    },
    editSheetActionBar: {
      borderTopWidth: 1,
      paddingHorizontal: 12,
      paddingTop: 10,
      paddingBottom: 10,
    },
    editSheetPrimaryButton: {
      minHeight: 50,
      borderRadius: ui.radius.button,
      alignItems: "center",
      justifyContent: "center",
      paddingHorizontal: 16,
      paddingVertical: 13,
    },
    editSheetPrimaryButtonText: {
      fontSize: 14,
      lineHeight: 18,
      fontWeight: "900",
      letterSpacing: 0,
    },

    list: {
      gap: 10,
      marginTop: 0,
      marginBottom: 0,
      paddingTop: 0,
      borderTopWidth: 0,
      borderTopColor: "transparent",
    },
    questQueueGroup: {
      gap: 8,
      paddingVertical: 2,
    },
    questQueueHeader: {
      flexDirection: "row",
      alignItems: "center",
      justifyContent: "space-between",
      gap: ui.spacing.sm,
      paddingHorizontal: 1,
    },
    questQueueTitleRow: {
      flexDirection: "row",
      alignItems: "center",
      gap: 7,
      flex: 1,
      minWidth: 0,
    },
    questQueueDot: {
      width: 8,
      height: 8,
      borderRadius: 999,
      flexShrink: 0,
    },
    questQueueTitle: {
      color: colors.textPrimary,
      fontSize: 12,
      lineHeight: 16,
      fontWeight: "900",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    questQueueMeta: {
      borderWidth: 1,
      borderRadius: 999,
      paddingHorizontal: 8,
      paddingVertical: 3,
      fontSize: 11,
      lineHeight: 14,
      fontWeight: "900",
      letterSpacing: 0,
      overflow: "hidden",
    },
    questQueueList: {
      gap: 7,
    },
    questCard: {
      backgroundColor: withAlpha(colors.surface2, 0.72),
      borderWidth: 1,
      borderRadius: ui.radius.card,
      borderColor: withAlpha(colors.border, 0.24),
      paddingVertical: 11,
      paddingHorizontal: 12,
      position: "relative",
      overflow: "hidden",
    },
    questContract: {
      backgroundColor: withAlpha(CONTRACT_GOLD, 0.07),
      borderColor: withAlpha(CONTRACT_GOLD, 0.32),
    },
    questContractRail: {
      position: "absolute",
      left: 0,
      top: 0,
      bottom: 0,
      width: 4,
      borderRadius: 0,
      backgroundColor: withAlpha(CONTRACT_GOLD, 0.78),
    },
    questCompletionFlashOverlay: {
      ...StyleSheet.absoluteFillObject,
      borderRadius: ui.radius.card,
    },
    questDivider: {
      height: 1,
      backgroundColor: withAlpha(colors.border, 0.22),
      marginVertical: 8,
    },
    questHeader: {
      flexDirection: "row",
      alignItems: "center",
      gap: 7,
      paddingHorizontal: 0,
      paddingVertical: 0,
    },
    questCompleteToggle: {
      width: 44,
      height: 44,
      alignItems: "center",
      justifyContent: "center",
    },
    questCheckBox: {
      width: 28,
      height: 28,
      borderRadius: 14,
      borderWidth: 2,
      alignItems: "center",
      justifyContent: "center",
    },
    questCompleteIconWrap: {
      width: 20,
      height: 20,
      alignItems: "center",
      justifyContent: "center",
    },
    questCompleteAnimatedCheck: {
      position: "absolute",
      alignItems: "center",
      justifyContent: "center",
    },
    questHeaderMain: {
      flex: 1,
      flexDirection: "row",
      alignItems: "center",
      justifyContent: "space-between",
      gap: 8,
    },
    questHeaderPressed: {
      opacity: 0.82,
    },
    questMetaRow: {
      flexDirection: "row",
      alignItems: "center",
      gap: ui.spacing.xs,
      marginTop: ui.spacing.xxs,
    },
    questMetaDifficulty: {
      fontSize: 10,
      fontWeight: "700",
      letterSpacing: 0,
      textTransform: "uppercase",
      opacity: 0.82,
    },
    questMetaCategory: {
      fontSize: 11,
      fontWeight: "600",
      lineHeight: 15,
      opacity: 0.76,
    },
    questMetaSingleLine: {
      marginTop: 3,
      fontSize: 12,
      fontWeight: "600",
      lineHeight: 17,
      opacity: 0.82,
    },
    questStatusStack: {
      alignItems: "flex-end",
      gap: 6,
      marginLeft: ui.spacing.xs,
      minWidth: 34,
    },
    questArtBadge: {
      width: 40,
      height: 40,
      borderRadius: 13,
      borderWidth: 1,
      alignItems: "center",
      justifyContent: "center",
      flexShrink: 0,
      position: "relative",
    },
    questArtGlyph: {
      fontSize: 19,
      lineHeight: 23,
    },
    questArtGlyphDone: {
      opacity: 0.58,
    },
    questDoneMark: {
      position: "absolute",
      right: -3,
      bottom: -3,
      width: 16,
      height: 16,
      borderRadius: 8,
      borderWidth: 1,
      alignItems: "center",
      justifyContent: "center",
    },
    statusPill: {
      paddingVertical: 4,
      paddingHorizontal: 7,
      borderRadius: 999,
      fontSize: 9,
      fontWeight: "900",
      letterSpacing: 0,
      overflow: "hidden",
    },
    questChevron: {
      opacity: 0.72,
    },
    questChevronOpen: {
      transform: [{ rotate: "90deg" }],
    },
    questActionsRow: {
      flexDirection: "row",
      gap: 8,
      alignItems: "center",
      justifyContent: "space-between",
      paddingTop: 0,
    },
    questSecondaryActionsRow: {
      flexDirection: "row",
      gap: 8,
      alignItems: "center",
      paddingTop: 8,
    },
    questActionBtnSubtle: {
      paddingVertical: 5,
      paddingHorizontal: 10,
      borderRadius: 999,
      borderWidth: 1,
      minWidth: 58,
      minHeight: 44,
      flex: 1,
      alignItems: "center",
      justifyContent: "center",
    },
    questActionToolBtn: {
      width: 44,
      height: 44,
      borderRadius: ui.radius.button,
      borderWidth: 1,
      alignItems: "center",
      justifyContent: "center",
      flexShrink: 0,
    },
    questActionBtnPrimary: {
      flex: 1,
      height: 44,
      paddingVertical: 5,
      paddingHorizontal: 12,
      borderRadius: ui.radius.button,
      borderWidth: 1,
      minWidth: 104,
      alignItems: "center",
      justifyContent: "center",
    },
    questActionTextSubtle: { fontWeight: "800", fontSize: 11, letterSpacing: 0 },
    questActionTextPrimary: { fontWeight: "800", fontSize: 11, letterSpacing: 0 },
    questActionBtn: {
      paddingVertical: 8,
      paddingHorizontal: 12,
      borderRadius: 999,
      borderWidth: 1,
    },
    questActionText: { fontWeight: "900", fontSize: 12 },
    questDone: {
      opacity: 0.76,
      borderColor: withAlpha(colors.border, 0.24),
      backgroundColor: withAlpha(colors.surface2, 0.42),
    },
    questDoneBar: {
      height: 1,
      borderRadius: 999,
      backgroundColor: `${colors.accentSecondary}55`,
      marginBottom: ui.spacing.xs,
    },
    questTitle: { color: colors.textPrimary, fontSize: 15, lineHeight: 19, fontWeight: "900" },
    questTitleDone: { textDecorationLine: "line-through", opacity: 0.7 },
    questMetaDone: { opacity: 0.58 },
    questMeta: { color: colors.textSecondary, marginTop: 6, fontSize: 12 },

    rightStack: { gap: 9, alignItems: "flex-end" },

    completeBtn: {
      backgroundColor: colors.accentPrimary,
      paddingVertical: 9,
      paddingHorizontal: 12,
      borderRadius: ui.radius.button,
      shadowColor: colors.accentPrimary,
      shadowOffset: { width: 0, height: 2 },
      shadowOpacity: 0.16,
      shadowRadius: 4,
    },
    completeText: { color: colors.textPrimary, fontWeight: "900", fontSize: 11 },

    doneBadge: {
      backgroundColor: `${colors.accentSecondary}1A`,
      borderWidth: 1,
      borderColor: withAlpha(colors.accentSecondary, 0.42),
      paddingVertical: 9,
      paddingHorizontal: 11,
      borderRadius: ui.radius.button,
    },
    doneText: { color: colors.accentSecondary, fontWeight: "900", fontSize: 11 },

    editBtn: {
      backgroundColor: colors.bg,
      borderWidth: 1,
      borderColor: withAlpha(colors.border, 0.3),
      paddingVertical: 7,
      paddingHorizontal: 11,
      borderRadius: ui.radius.button,
    },
    editText: { color: colors.textPrimary, fontWeight: "900", fontSize: 11 },

    pinBtn: {
      backgroundColor: colors.bg,
      borderWidth: 1,
      borderColor: withAlpha(colors.border, 0.3),
      paddingVertical: 7,
      paddingHorizontal: 11,
      borderRadius: ui.radius.button,
    },
    pinText: { color: colors.textSecondary, fontWeight: "900", fontSize: 11 },

    deleteBtn: {
      backgroundColor: colors.bg,
      borderWidth: 1,
      borderColor: withAlpha(colors.accentPrimary, 0.35),
      paddingVertical: 7,
      paddingHorizontal: 11,
      borderRadius: ui.radius.button,
    },
    deleteText: { color: colors.accentPrimary, fontWeight: "900", fontSize: 11 },

    btnPressed: {
      opacity: 0.7,
      transform: [{ scale: 0.95 }],
    },

    grid: { gap: 12 },
    card: {
      ...cardSurface,
      padding: ui.spacing.card,
      borderRadius: ui.radius.card,
    },
    cardSecondary: {
      ...secondaryCardSurface,
      padding: ui.spacing.md,
    },
    focusList: { marginTop: ui.spacing.sm },
    focusRow: {
      flexDirection: "row",
      justifyContent: "space-between",
      alignItems: "center",
      paddingVertical: ui.spacing.sm,
      paddingHorizontal: 0,
      backgroundColor: "transparent",
      borderRadius: 0,
      borderWidth: 0,
      borderColor: "transparent",
    },
    focusLabel: {
      ...ui.typography.body,
      color: colors.textPrimary,
      fontWeight: "700",
      opacity: 0.9,
    },
    focusMeta: {
      ...ui.typography.caption,
      marginTop: 0,
      color: withAlpha(colors.textSecondary, 0.88),
      fontWeight: "700",
      textAlign: "right",
    },
    focusDivider: {
      height: 1,
      backgroundColor: withAlpha(colors.border, 0.28),
    },
    cardTop: {
      flexDirection: "row",
      justifyContent: "space-between",
      alignItems: "center",
      marginBottom: 9,
    },
    cardTitle: { ...ui.typography.heading, color: colors.textPrimary, fontSize: 18 },
    drSystemSubhead: {
      color: colors.textSecondary,
      fontSize: 11,
      fontWeight: "800",
      letterSpacing: 0,
      textTransform: "uppercase",
      marginTop: 14,
      marginBottom: 8,
    },
    drSystemTopRow: {
      backgroundColor: withAlpha(colors.accentPrimary, 0.07),
      borderWidth: ui.border.width,
      borderColor: withAlpha(colors.accentPrimary, 0.22),
      borderRadius: ui.radius.md,
      paddingVertical: ui.spacing.xs,
      paddingHorizontal: ui.spacing.sm,
      marginBottom: ui.spacing.sm,
    },
    drSystemTopText: {
      color: colors.textPrimary,
      fontSize: 13,
      fontWeight: "700",
      lineHeight: 18,
    },
    drSystemRow: {
      ...compactTileSurface,
      borderWidth: ui.border.width,
      borderRadius: ui.radius.md,
      paddingVertical: ui.spacing.xs,
      paddingHorizontal: ui.spacing.sm,
      marginTop: 6,
      flexDirection: "row",
      justifyContent: "space-between",
      alignItems: "center",
    },
    drSystemRowActive: {
      borderColor: withAlpha(colors.accentPrimary, 0.35),
      backgroundColor: withAlpha(colors.accentPrimary, 0.09),
    },
    statsSectionFlat: {
      marginBottom: ui.spacing.md,
    },
    analyticsRow: {
      flexDirection: "row",
      gap: ui.spacing.xs,
      marginBottom: ui.spacing.md,
    },
    analyticsTile: {
      ...compactTileSurface,
      flex: 1,
      paddingVertical: ui.spacing.xs,
      paddingHorizontal: ui.spacing.xs,
    },
    analyticsLabel: {
      ...ui.typography.caption,
      color: colors.textSecondary,
      fontSize: 9,
      letterSpacing: 0,
    },
    analyticsValue: {
      ...ui.typography.mono,
      color: colors.textPrimary,
      fontSize: 15,
      marginTop: 2,
    },
    trendCard: {
      ...cardSurface,
      borderColor: withAlpha(colors.border, 0.24),
      backgroundColor: withAlpha(colors.surface2, 0.72),
      borderRadius: ui.radius.card,
      paddingHorizontal: ui.spacing.md,
      paddingVertical: ui.spacing.md,
      marginBottom: ui.spacing.md,
      gap: ui.spacing.xs,
    },
    trendHeaderRow: {
      flexDirection: "row",
      justifyContent: "space-between",
      alignItems: "center",
      gap: ui.spacing.sm,
    },
    trendTitle: {
      color: colors.textPrimary,
      fontSize: 17,
      lineHeight: 21,
      fontWeight: "900",
    },
    trendMeta: {
      color: withAlpha(colors.textSecondary, 0.82),
      fontSize: 10,
      lineHeight: 15,
      fontWeight: "800",
      letterSpacing: 0,
      textTransform: "uppercase",
    },
    trendBarRail: {
      height: 58,
      flexDirection: "row",
      alignItems: "flex-end",
      gap: 6,
      marginTop: ui.spacing.xs,
    },
    trendBar: {
      flex: 1,
      minHeight: 4,
      borderRadius: 999,
      backgroundColor: colors.accentPrimary,
      opacity: 0.88,
    },
    streakGrid: {
      flexDirection: "row",
      gap: ui.spacing.xs,
      marginBottom: ui.spacing.md,
    },
    streakTile: {
      ...compactTileSurface,
      flex: 1,
      paddingHorizontal: ui.spacing.xs,
      paddingVertical: ui.spacing.sm,
      minWidth: 0,
    },
    streakValue: {
      color: colors.textPrimary,
      fontSize: 24,
      lineHeight: 28,
      fontWeight: "900",
    },
    streakLabel: {
      color: withAlpha(colors.textSecondary, 0.8),
      fontSize: 9,
      lineHeight: 12,
      fontWeight: "800",
      letterSpacing: 0,
      textTransform: "uppercase",
      marginTop: 2,
    },
    statsSectionDivider: {
      height: 1,
      backgroundColor: withAlpha(colors.border, 0.32),
      marginTop: ui.spacing.sm,
      marginBottom: ui.spacing.md,
    },
    tableHeaderRow: {
      flexDirection: "row",
      paddingVertical: ui.spacing.xs,
      borderBottomWidth: 1,
      borderBottomColor: withAlpha(colors.border, 0.48),
      marginBottom: ui.spacing.xxs,
    },
    tableHeaderText: {
      ...ui.typography.caption,
      color: withAlpha(colors.textSecondary, 0.9),
      fontSize: 10,
      letterSpacing: 0,
    },
    tableRow: {
      flexDirection: "row",
      alignItems: "center",
      paddingVertical: ui.spacing.xs,
      borderBottomWidth: 1,
      borderBottomColor: withAlpha(colors.border, 0.28),
    },
    tableCell: {
      color: colors.textPrimary,
      fontSize: 12,
      fontWeight: "700",
    },
    tableCellSubtext: {
      color: withAlpha(colors.textSecondary, 0.78),
      fontSize: 10,
      lineHeight: 15,
      fontWeight: "700",
      marginTop: 1,
    },
    drRankRow: {
      backgroundColor: colors.surface,
      borderWidth: 0,
      borderRadius: ui.radius.md,
      paddingVertical: ui.spacing.sm,
      paddingHorizontal: ui.spacing.sm,
      marginTop: ui.spacing.sm,
      minHeight: 52,
      flexDirection: "row",
      alignItems: "center",
      shadowColor: colors.text,
      shadowOffset: ui.shadow.soft.shadowOffset,
      shadowOpacity: 0.07,
      shadowRadius: ui.shadow.soft.shadowRadius,
      elevation: ui.shadow.soft.elevation,
    },
    drRankRowActive: {
      backgroundColor: withAlpha(colors.accentPrimary, 0.1),
    },
    drRankAccentBar: {
      width: 3,
      alignSelf: "stretch",
      borderRadius: 999,
      marginRight: ui.spacing.sm,
      backgroundColor: colors.accentPrimary,
    },
    drRankRowContent: {
      flex: 1,
      flexDirection: "row",
      justifyContent: "space-between",
      alignItems: "center",
    },
    drSystemRankName: {
      color: colors.textPrimary,
      fontSize: 13,
      fontWeight: "700",
    },
    drSystemRankRange: {
      color: colors.textSecondary,
      fontSize: 12,
      fontWeight: "800",
    },
    drSystemRuleLabel: {
      color: colors.textPrimary,
      fontSize: 12,
      fontWeight: "700",
    },
    drSystemRuleDelta: {
      fontSize: 12,
      fontWeight: "900",
    },
    level: { fontSize: 14, fontWeight: "700", color: colors.accentPrimary, letterSpacing: 0 },
    xpText: { color: colors.textSecondary, marginBottom: 9, fontSize: 12, fontWeight: "500" },
    barTrack: {
      height: 11,
      borderRadius: 999,
      backgroundColor: colors.bg,
      overflow: "hidden",
      borderWidth: 1,
      borderColor: withAlpha(colors.border, 0.3),
    },
    barFill: { height: "100%", backgroundColor: colors.accentPrimary, borderRadius: 999 },

    difficultyEasy: {
      backgroundColor: `${colors.accentTertiary}1A`,
      borderWidth: 1,
      borderColor: withAlpha(colors.accentTertiary, 0.45),
      paddingVertical: 5,
      paddingHorizontal: 9,
      borderRadius: 999,
    },
    difficultyMedium: {
      backgroundColor: `${colors.accentSecondary}1A`,
      borderWidth: 1,
      borderColor: withAlpha(colors.accentSecondary, 0.45),
      paddingVertical: 5,
      paddingHorizontal: 9,
      borderRadius: 999,
    },
    difficultyHard: {
      backgroundColor: `${colors.accentPrimary}1A`,
      borderWidth: 1,
      borderColor: withAlpha(colors.accentPrimary, 0.45),
      paddingVertical: 5,
      paddingHorizontal: 9,
      borderRadius: 999,
    },
    difficultyText: { fontWeight: "900", fontSize: 10, color: colors.textPrimary, letterSpacing: 0 },

    // Achievements
    achievementSection: { marginBottom: 28, marginTop: 28 },
    achievementHeader: {
      flexDirection: "row",
      justifyContent: "space-between",
      alignItems: "center",
      marginBottom: 16,
    },
    achievementProgress: {
      backgroundColor: withAlpha(colors.surface2, 0.7),
      borderRadius: 999,
      paddingVertical: 6,
      paddingHorizontal: 12,
      borderWidth: 1,
      borderColor: withAlpha(colors.border, 0.24),
    },
    achievementProgressText: { color: colors.accentPrimary, fontWeight: "800", fontSize: 13 },
    achievementGrid: { gap: 12 },
    achievementBadge: {
      ...cardSurface,
      padding: ui.spacing.md,
      alignItems: "center",
      gap: 8,
    },
    achievementUnlocked: {
      backgroundColor: `${colors.accentSecondary}1A`,
      borderColor: withAlpha(colors.accentSecondary, 0.38),
      shadowColor: colors.accentSecondary,
      shadowRadius: 6,
    },
    achievementLocked: {
      opacity: 0.4,
      backgroundColor: colors.bg,
      borderColor: withAlpha(colors.border, 0.24),
    },
    achievementIcon: { fontSize: 32 },
    achievementName: { color: colors.textPrimary, fontWeight: "900", fontSize: 13, textAlign: "center" },
    achievementDesc: { color: colors.textSecondary, fontWeight: "600", fontSize: 12, textAlign: "center" },
    achievementDate: { color: colors.accentSecondary, fontWeight: "800", fontSize: 11, marginTop: 4 },

    hint: { marginTop: 24, color: colors.textSecondary, lineHeight: 20, fontSize: 13 },
    emptyQuestCard: {
      paddingVertical: ui.spacing.lg,
      paddingHorizontal: ui.spacing.sm,
      gap: ui.spacing.xs,
      alignItems: "flex-start",
    },
    emptyQuestTitle: {
      color: colors.textPrimary,
      fontSize: 18,
      lineHeight: 22,
      fontWeight: "900",
    },
    emptyQuestText: {
      color: withAlpha(colors.textSecondary, 0.82),
      fontSize: 13,
      lineHeight: 18,
      fontWeight: "700",
      maxWidth: 310,
    },
    emptyQuestButton: {
      marginTop: ui.spacing.xs,
      backgroundColor: colors.accentPrimary,
      borderRadius: ui.radius.button,
      paddingVertical: 10,
      paddingHorizontal: 13,
      minHeight: 44,
      justifyContent: "center",
    },
    emptyQuestActions: {
      flexDirection: "row",
      flexWrap: "wrap",
      justifyContent: "center",
      gap: 8,
      marginTop: 4,
    },
    emptyQuestButtonSecondary: {
      backgroundColor: withAlpha(HOME_GOLD, 0.08),
      borderWidth: 1,
      borderColor: withAlpha(HOME_GOLD, 0.38),
    },
    emptyQuestButtonTextSecondary: { color: HOME_GOLD },
    reflectionCard: {
      marginTop: ui.spacing.md,
      borderWidth: 1,
      borderColor: withAlpha(HOME_GOLD, 0.24),
      borderRadius: ui.radius.card,
      backgroundColor: withAlpha(colors.surface2, 0.62),
      padding: ui.spacing.md,
      gap: 10,
    },
    reflectionHeaderRow: {
      flexDirection: "row",
      alignItems: "flex-start",
      justifyContent: "space-between",
      gap: 12,
    },
    reflectionHeaderCopy: { flex: 1 },
    reflectionEyebrow: {
      color: HOME_GOLD,
      fontSize: 10,
      lineHeight: 14,
      fontWeight: "900",
      textTransform: "uppercase",
    },
    reflectionTitle: {
      color: colors.textPrimary,
      fontSize: 18,
      lineHeight: 22,
      fontWeight: "900",
      marginTop: 2,
    },
    reflectionCount: {
      color: colors.textSecondary,
      fontSize: 10,
      lineHeight: 14,
      fontWeight: "800",
    },
    reflectionHelp: {
      color: colors.textSecondary,
      fontSize: 12,
      lineHeight: 17,
      fontWeight: "700",
    },
    previousReflection: {
      borderLeftWidth: 2,
      borderLeftColor: withAlpha(HOME_GOLD, 0.48),
      paddingLeft: 10,
      gap: 2,
    },
    previousReflectionLabel: {
      color: HOME_GOLD,
      fontSize: 9,
      lineHeight: 13,
      fontWeight: "900",
      textTransform: "uppercase",
    },
    previousReflectionText: {
      color: colors.textSecondary,
      fontSize: 11,
      lineHeight: 16,
      fontWeight: "700",
    },
    reflectionInput: {
      minHeight: 92,
      borderWidth: 1,
      borderColor: withAlpha(colors.border, 0.42),
      borderRadius: 10,
      backgroundColor: withAlpha(colors.bg, 0.5),
      color: colors.textPrimary,
      paddingHorizontal: 11,
      paddingVertical: 10,
      fontSize: 13,
      lineHeight: 19,
    },
    reflectionFooter: {
      flexDirection: "row",
      alignItems: "center",
      justifyContent: "space-between",
      gap: 12,
    },
    reflectionSavedText: {
      flex: 1,
      color: colors.textSecondary,
      fontSize: 10,
      lineHeight: 14,
      fontWeight: "700",
    },
    reflectionSaveButton: {
      minWidth: 104,
      minHeight: 44,
      borderRadius: ui.radius.button,
      backgroundColor: HOME_GOLD,
      alignItems: "center",
      justifyContent: "center",
      paddingHorizontal: 13,
    },
    reflectionSaveButtonDisabled: { opacity: 0.42 },
    reflectionSaveButtonText: {
      color: colors.bg,
      fontSize: 11,
      lineHeight: 15,
      fontWeight: "900",
    },
    emptyQuestButtonText: {
      color: colors.bg,
      fontSize: 12,
      lineHeight: 15,
      fontWeight: "900",
      letterSpacing: 0,
    },
    homeHintCard: {
      marginTop: ui.spacing.sm,
      paddingVertical: ui.spacing.xs,
      paddingHorizontal: 0,
      borderTopWidth: 1,
      borderTopColor: withAlpha(colors.border, 0.28),
    },
    homeHintText: {
      color: withAlpha(colors.textSecondary, 0.84),
      lineHeight: ui.typography.body.lineHeight,
      fontSize: ui.typography.body.fontSize,
      fontWeight: "600",
      opacity: 0.9,
    },
  });
};
